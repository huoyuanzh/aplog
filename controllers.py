...
class category:
    def GET(self, arg):
        ...
        if not term:
            raise web.notfound()
        posts = term.posts
        return render.archive({'archtype': 'category', 'name': term.name,
                               'posts': posts, 'page': page, 'widget': get_sidebar()})
...
class tag:
    def GET(self, arg):
        ...
        return render.archive({'archtype': 'tag', 'name': term.name,
...
class archives:
    ...
    def GET(self, year, month):
        ...
        if year and month:
            ...
            return render.archive({'archtype': 'archive', 'year': year, 'month': month,
                                   'posts': posts, 'page': page, 'widget': get_sidebar()})
        else:
            raise web.notfound()
